...
def get_data(file, type):
    ...
    return torch.tensor(np.array(four_day_datas)),torch.tensor(close),four_day_nor
...
train_x,train_y,train_nor = get_data('./Dataset/train.csv','train')
...
### Test ###
val_x,val_y,val_nor = get_data('./Dataset/train.csv','val')
...
for rate,model in models.items():
    ...
    loss = loss_fc(y_predicted, val_y)
...
test_x,_,test_nor = get_data('./Dataset/test.csv','all')
...
for i,t in enumerate(y_predicted.flatten()):
    write_data += f'{i},{test_nor[i](t.item())}\n'
